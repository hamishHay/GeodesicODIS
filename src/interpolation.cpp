...
// Function to find the tangential velocity component
// at each face via interpolation 
int interpolateVelocity(Globals *globals,
                        Mesh *mesh,
                        Array2D<double> &interp_vel,
                        Array1D<double> &normal_vel)
{
    double v_tang, v_norm;      // Velocity components
    int n1, n2;                 // Neighbouring nodes to face
    int fnum[2];                // Number of friends of n1 and n2
                       ...
                       double beta = 1.0)
{
    ...
    for (int i = 0; i < FACE_NUM; ++i)
    {
        ...
        flux(i) = vel * 0.5 * (node_scalar(inner_ID) + node_scalar(outer_ID)) - dx2 * (d2_outer + d2_inner) * vel + dx2 * beta * fabs(vel) * (d2_outer - d2_inner);
    }
    return 1;
}
